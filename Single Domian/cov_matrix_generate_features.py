...
# 处理每个类和客户端
def process_class_client(class_idx, client_idx, cov_matrices, initial_features_dir, complete_features_dir):
    ...
    if num_samples_needed > 0:
        if num_samples_needed <= num_samples_current:
            ...
            for feature in class_features:
                new_samples = generate_new_samples(feature, cov_matrices[class_idx], num_generated_per_sample)  # 基于每个原始样本生成新样本
                new_samples_list.append(new_samples)
...
def main():
    ...
    for class_idx in range(10):
        for client_idx in tqdm(range(10), desc=f'Processing class {class_idx}', leave=False):
            process_class_client(class_idx, client_idx, cov_matrices, initial_features_dir, complete_features_dir)
