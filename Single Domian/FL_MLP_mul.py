...
def train_on_client(features, labels, global_model_path, num_classes, epoch=10, validation_data=None):
    model = MyNet(num_classes=num_classes).to(device)
    ...
    for e in range(epoch):
        ...
        if validation_data:
            ...
            accuracy, _ = evaluate_model(model, val_features, val_labels)
    ...
    if best_model_state:
        torch.save(best_model_state, client_model_path)
    else:
...
def extract_image_features(model, data, indices, preprocess):
    ...
    with torch.no_grad():
        for index in indices:
            img_array = data[index].numpy().transpose((1, 2, 0))
...
def main():
    ...
    output_dir = f'./CIFAR-10/features/alpha={alpha}_model/Fedavg_num_rounds={num_rounds}_epoch={epoch}'
    ...
    for round in range(num_rounds):
        ...
        for client_id in client_ids:
            ...
            client_model_path = train_on_client(features, labels, 'original_global_initial_model.pth', num_classes, epoch, validation_data=(test_features, test_labels))
        ...
        round_dir = f'./CIFAR-10/features/alpha={alpha}_model/Fedavg_num_rounds={num_rounds}_epoch={epoch}/recent_round={round+1}'
        ...
        print(f"Round {round + 1} - Original feature model's CIFAR-10 test accuracy: {original_accuracy:.4f}")
        print(f"Round {round + 1} - Original feature model's CIFAR-10 classification report:\n{original_report}")
        original_accuracies.append(original_accuracy)
        with open(os.path.join(round_dir, 'original_report.txt'), 'w') as f:
            f.write(f"Original feature model's CIFAR-10 test accuracy: {original_accuracy:.4f}\n")
            f.write(f"Original feature model's CIFAR-10 classification report:\n{original_report}\n")
    ...
    for round in range(num_rounds):
        ...
        for client_id in client_ids:
            ...
            client_model_path = train_on_client(features, labels, 'augmented_global_initial_model.pth', num_classes, epoch, validation_data=(test_features, test_labels))
            client_model_paths.append(client_model_path)
        ...
        print(f"Round {round + 1} - Augmented feature model's CIFAR-10 test accuracy: {augmented_accuracy:.4f}")
        print(f"Round {round + 1} - Augmented feature model's CIFAR-10 classification report:\n{augmented_report}")
        augmented_accuracies.append(augmented_accuracy)
        with open(os.path.join(round_dir, 'augmented_report.txt'), 'w') as f:
            f.write(f"Augmented feature model's CIFAR-10 test accuracy: {augmented_accuracy:.4f}\n")
            f.write(f"Augmented feature model's CIFAR-10 classification report:\n{augmented_report}\n")
