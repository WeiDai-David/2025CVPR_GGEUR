...
class MyNet(nn.Module):
    def __init__(self, num_classes=100):  # CIFAR-100有100个类别
...
def load_client_features(client_idx, base_dir, alpha):
    ...
    for class_idx in range(100):  # CIFAR-100 有100个类别
...
def train_and_evaluate(train_dict, model_path, model_name, test_dataloader, output_file_path):
    train_dataset = MyDataset(train_dict['features'], train_dict['labels'])
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    model = MyNet(num_classes=100).to(device)  # CIFAR-100 有100个类别
    ...
    for epoch in range(num_epochs):
        ...
        test_accuracy, _ = evaluate_model(model, test_dataloader, device)

        train_accuracies.append(train_accuracy * 100)  # 保存训练精度（百分比）
    ...
    test_accuracy, report = evaluate_model(model, test_dataloader, device)
    output_content = (
...
# 画精度变化图
def plot_accuracies(train_accuracies, test_accuracies, model_name, output_dir):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.title(f'{model_name} Accuracy over Epochs (Default Range)')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, f'{model_name}_accuracy_default_range.png'))
    plt.close()
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.title(f'{model_name} Accuracy over Epochs (0%-100%)')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(output_dir, f'{model_name}_accuracy_0_to_100.png'))
    plt.close()
...
test_features_dir = f'./CIFAR-100/features/resnet_test'
...
# 设置模型保存路径
model_dir = f'./CIFAR-100/features/resnet_alpha={alpha}_model/client_{client_idx}'
os.makedirs(model_dir, exist_ok=True)

# 训练和评估原始特征模型
original_model_path = os.path.join(model_dir, 'original_cifar100_best_model.pth')
train_accuracies, test_accuracies = train_and_evaluate({'features': original_features, 'labels': original_labels}, original_model_path, "原始特征模型", test_dataloader, os.path.join(model_dir, 'original_report.txt'))

# 绘制原始特征模型的精度变化图
plot_accuracies(train_accuracies, test_accuracies, "original_model", model_dir)

# 训练和评估补全特征模型
augmented_model_path = os.path.join(model_dir, 'augmented_cifar100_best_model.pth')
train_accuracies, test_accuracies = train_and_evaluate({'features': augmented_features, 'labels': augmented_labels}, augmented_model_path, "补全特征模型", test_dataloader, os.path.join(model_dir, 'augmented_report.txt'))

# 绘制补全特征模型的精度变化图
plot_accuracies(train_accuracies, test_accuracies, "augmented_model", model_dir)
