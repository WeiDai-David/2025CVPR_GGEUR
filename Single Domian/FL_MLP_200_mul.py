...
def main():
    ...
    test_features_dir = f'./TinyImageNet/features/alpha={alpha}_test'
    output_dir = f'./TinyImageNet/features/alpha={alpha}_model/Fedavg_num_rounds={num_rounds}_epoch={epoch}'
    ...
    if test_features is None or test_labels is None:
        ...
        test_dataset = datasets.ImageFolder(root='./data/tiny-imagenet-200/test', transform=transform)
        test_data, test_labels = zip(*[(data, label) for data, label in test_dataset])
        test_data = np.array([np.array(d).transpose(1, 2, 0) for d in test_data])  # 转换为图像格式
    ...
    for round in range(num_rounds):
        ...
        for client_id in client_ids:
            features, labels = load_features('./TinyImageNet/features/initial', alpha, num_classes, is_filled=False)
    ...
    for round in range(num_rounds):
        ...
        client_model_paths = []
        for client_id in client_ids:
            features, labels = load_features(f'./TinyImageNet/features/alpha={alpha}_complete', alpha, num_classes, is_filled=True)
    ...
    print(f"Original feature model's TinyImageNet test accuracy: {original_accuracy:.4f}")
    print(f"Original feature model's TinyImageNet classification report:\n{original_report}")
    original_output_content = (
        f"Original feature model's TinyImageNet test accuracy: {original_accuracy:.4f}\n"
        f"Original feature model's TinyImageNet classification report:\n{original_report}\n"
        )
    ...
    print(f"Augmented feature model's TinyImageNet test accuracy: {augmented_accuracy:.4f}")
    print(f"Augmented feature model's TinyImageNet classification report:\n{augmented_report}")
    augmented_output_content = (
        f"Augmented feature model's TinyImageNet test accuracy: {augmented_accuracy:.4f}\n"
        f"Augmented feature model's TinyImageNet classification report:\n{augmented_report}\n"
        )
    with open(output_dir, 'a') as f:
