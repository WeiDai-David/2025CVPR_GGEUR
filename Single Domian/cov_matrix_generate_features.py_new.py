# cov_matrix_generate_features.py 该脚本用聚合协方差依托原始特征为基础生成补全特征
import numpy as np
import os
...
# 处理每个类和客户端
def process_class_client(class_idx, client_idx, cov_matrices, initial_features_dir, complete_features_dir ,alpha ):
    ...
    if num_samples_needed > 0:
        if num_samples_needed <= num_samples_current:
            ...
            for feature in class_features:
                new_samples = generate_new_samples(feature, cov_matrices[class_idx], 500)  # 每个现有样本生成500个新样本
                new_samples_list.append(new_samples)
            new_samples_flattened = np.vstack(new_samples_list)
...
def main():
    alpha = 0.5
    base_dir = './CIFAR-10/features/'  # 基础目录
    ...
    for class_idx in range(10):
        for client_idx in tqdm(range(10), desc=f'Processing class {class_idx}', leave=False):
            process_class_client(class_idx, client_idx, cov_matrices, initial_features_dir, complete_features_dir ,alpha)
