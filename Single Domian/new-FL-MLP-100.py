...
batch_size = 64
learning_rate = 0.001
# communication_rounds = 50  # 通信轮数
# local_epochs = 10  # 每轮通信的本地epoch数
...
# 保存原始特征模型
original_model_path = f'./CIFAR-100/features/alpha={alpha}_model/global_original_model.pth'
...
plt.legend()
plt.savefig(f'./CIFAR-100/features/alpha={alpha}_model/original_test_accuracies.png')
plt.show()
...
# 保存补全特征模型
augmented_model_path = f'./CIFAR-100/features/alpha={alpha}_model/global_augmented_model.pth'
...
plt.title('Test Accuracy vs Communication Rounds')
plt.legend()
plt.savefig(f'./CIFAR-100/features/alpha={alpha}_model/augmented_test_accuracies.png')
plt.show()
