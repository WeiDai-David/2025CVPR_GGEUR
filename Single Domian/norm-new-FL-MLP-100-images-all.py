...
def calculate_and_save_weight_norms(model, round_num, alpha, client_num=None, save_global=False, feature_type="original"):
    ...
    if save_global:
        ...
        save_dir = f'./CIFAR-100/clip_fedavg_alpha_{alpha}/{feature_type}/global'
    else:
        plt.title(f'Client {client_num} Round {round_num}')
        save_dir = f'./CIFAR-100/clip_fedavg_alpha_{alpha}/{feature_type}/client{client_num}'
...
def load_client_features(client_idx, base_dir, alpha):
    ...
    for class_idx in range(100):  # CIFAR-100 有 100 类
...
test_features_dir = f'./CIFAR-100/features/alpha={alpha}_test'  # 修改为 CIFAR-100
...
if test_features is None or test_labels is None:
    ...
    test_data_batch_file = './data/cifar-100-python/test'  # 修改为 CIFAR-100 批次文件
    ...
    test_labels = np.array(test_data_batch[b'fine_labels'])  # CIFAR-100 使用 fine_labels
...
def federated_train_and_evaluate(all_client_features, test_features, test_labels, communication_rounds, local_epochs, alpha, batch_size=64, learning_rate=0.001):
    ...
    report_path = f'./CIFAR-100/clip_fedavg_alpha_{alpha}/report.txt'
...
for idx in range(client_count):
    ...
    plt.savefig(f'./CIFAR-100/clip_fedavg_alpha_{alpha}/original/client{idx}_global_norm_ratios.png')
    plt.close()

# 保存原始特征模型
original_model_path = f'./CIFAR-100/clip_fedavg_alpha_{alpha}/model/global_original_model.pth'
...
plt.legend()
plt.savefig(f'./CIFAR-100//clip_fedavg_alpha_{alpha}/model/original_test_accuracies_0_100.png')
...
plt.title('Top-1 Accuracy vs Communication Rounds')
plt.legend()
plt.savefig(f'./CIFAR-100/clip_fedavg_alpha_{alpha}/model/original_test_accuracies.png')
...
def federated_train_and_evaluate_augmented(all_client_features, test_features, test_labels, communication_rounds, local_epochs, alpha, batch_size=64, learning_rate=0.001):
    ...
    test_accuracies = []
    report_path = f'./CIFAR-100/clip_fedavg_alpha_{alpha}/report_augmented.txt'
...
for idx in range(client_count):
    ...
    plt.legend()
    plt.title(f'Client {idx} and Global Model Norm Ratios')
    plt.savefig(f'./CIFAR-100/clip_fedavg_alpha_{alpha}/augmented/client{idx}_global_norm_ratios.png')
    plt.close()
    
# 保存补全特征模型
augmented_model_path = f'./CIFAR-100/clip_fedavg_alpha_{alpha}/model/global_augmented_model.pth'
...
plt.title('Top-1 Accuracy vs Communication Rounds')
plt.legend()
plt.savefig(f'./CIFAR-100/clip_fedavg_alpha_{alpha}/model/augmented_test_accuracies_0_100.png')
...
plt.title('Top-1 Accuracy vs Communication Rounds')
plt.legend()
plt.savefig(f'./CIFAR-100/clip_fedavg_alpha_{alpha}/model/augmented_test_accuracies.png')
