...
def combine_samples(original_features, expanded_features, other_generated_samples):
    combined_samples = []

    # 添加原始特征
    if original_features.shape[0] > 0:
        combined_samples.append(original_features)

    # 添加从原始特征生成的补全样本
    if expanded_features.shape[0] > 0:
        combined_samples.append(expanded_features)

    # 添加其他客户端生成的样本
    if other_generated_samples.shape[0] > 0:
        combined_samples.append(other_generated_samples)

    if len(combined_samples) > 0:
        final_samples = np.vstack(combined_samples)
    else:
        final_samples = np.empty((0, original_features.shape[1]))  # 空数组占位
...
def process_clients(client_id, class_idx, original_features, cov_matrices, prototype_features, num_per_sample):
    ...
    # 3. 合并原始样本、补全样本和其他客户端生成的样本
    final_samples = combine_samples(original_features, expanded_features, additional_samples)
