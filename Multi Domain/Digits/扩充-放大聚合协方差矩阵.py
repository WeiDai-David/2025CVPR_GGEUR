...
def process_clients(client_ids, cov_matrices, initial_features_dir, complete_features_dir, dataset_name, num_generated_per_sample, target_size):
    for client_id in client_ids:
        for class_idx in range(10):  # 假设有 10 个类别
...
def main():
    ...
    parser.add_argument('--base_dir', type=str, default='./clip_features/', help='Directory containing features.')
    ...
    # 定义不同数据集对应的客户端 ID 和生成样本的参数
    datasets_config = {
        'mnist': {'client_ids': [0], 'num_generated_per_sample': 100, 'target_size': 500},
        'usps': {'client_ids': [1], 'num_generated_per_sample': 100, 'target_size': 500},
        'svhn': {'client_ids': [2], 'num_generated_per_sample': 100, 'target_size': 500},
        'syn': {'client_ids': [3], 'num_generated_per_sample': 100, 'target_size': 500}
    }
