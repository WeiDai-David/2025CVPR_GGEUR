...
def federated_train_and_evaluate_scaffold(all_client_features, test_loaders, communication_rounds, local_epochs, batch_size=16, learning_rate=0.25):
    ...
    # 初始化全局控制变量和客户端控制变量
    global_control_var = [torch.zeros_like(param) for param in global_model.parameters()]
    client_control_vars = [[torch.zeros_like(param) for param in client_models[i].parameters()] for i in range(len(client_models))]
    ...
    report_path = './results/scaffold_report.txt'
...
def main():
    ...
    # 保存精度最高的模型
    best_model_path = './model/scaffold_best_global_model.pth'
    ...
    plt.legend()
    plt.savefig('./results/scaffold_test_accuracy_plot.png')
    plt.show()
