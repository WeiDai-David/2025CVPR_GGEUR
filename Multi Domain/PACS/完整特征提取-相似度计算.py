...
# 主函数
def main(datasets):
    output_base_dir = './clip_pacs_all_features'  # 保存特征的输出目录
    os.makedirs(output_base_dir, exist_ok=True)

    data_path = './data/PACS'  # PACS数据集的路径
    ...
    for dataset_name in datasets:
        ...
        for client_id in assigned_clients:
            ...
            dataset = load_pacs(domain_path)

            # 遍历每个类别
            for class_label in range(7):  # 假设PACS数据集中类别范围为0-6
                # 筛选出属于当前类别的图像
