...
def load_features(dataset_name, client_id, class_label):
    feature_path = f'./clip_pacs_all_features/{dataset_name}/client_{client_id}_class_{class_label}_original_features.npy'
...
def compute_similarity_for_datasets(dataset_name1, client_id1, dataset_name2, client_id2):
    similarity_matrix = np.zeros((7, 7))  # 假设PACS数据集有7个类别
    for i in range(7):
        for j in range(7):
...
# 处理 photo 和 art_painting 组合
process_and_save('photo', 'art_painting', output_dir)

# 处理 photo 和 cartoon 组合
process_and_save('photo', 'cartoon', output_dir)

# 处理 photo 和 sketch 组合
process_and_save('photo', 'sketch', output_dir)

# 处理 art_painting 和 cartoon 组合
process_and_save('art_painting', 'cartoon', output_dir)

# 处理 art_painting 和 sketch 组合
process_and_save('art_painting', 'sketch', output_dir)

# 处理 cartoon 和 sketch 组合
process_and_save('cartoon', 'sketch', output_dir)
