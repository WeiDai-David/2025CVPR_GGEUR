...
import open_clip
import argparse  # 添加 argparse 的导入
...
# Office-Caltech的预处理
transform_office_caltech = transforms.Compose([
    transforms.Resize((224, 224)),  # CLIP 使用的输入大小为224x224
])

# 加载Office-Caltech数据集
def load_office_caltech(domain_path):
    """
    加载Office-Caltech数据集的所有类和样本
    """
    dataset = datasets.ImageFolder(domain_path, transform=transform_office_caltech)
    return dataset
...
# 主函数
def main(datasets):
    output_base_dir = './clip_office_caltech_all_features'  # 保存特征的输出目录
    os.makedirs(output_base_dir, exist_ok=True)

    data_path = './data/Office_Caltech_10'  # Office-Caltech数据集的路径
    ...
    for dataset_name in datasets:
        ...
        for client_id in assigned_clients:
            ...
            dataset = load_office_caltech(domain_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--datasets', nargs='+', default=['caltech', 'amazon', 'webcam', 'dslr'], help='要处理的训练集')
