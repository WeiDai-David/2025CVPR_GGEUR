...
def federated_train_and_evaluate_fedprox(all_client_features, test_sets, communication_rounds, local_epochs, batch_size=16, learning_rate=0.001, mu=0.01):
    ...
    report_path = './results/FedProx_100-argumented_report.txt'
...
def plot_accuracies(accuracies, communication_rounds):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('./results/FedProx_100-argumented_test_accuracy_plot.png')
    plt.show()
...
def main():
    ...
    # 保存精度最高的模型
    best_model_path = './model/100-FedProx_argumented_best_model.pth'
