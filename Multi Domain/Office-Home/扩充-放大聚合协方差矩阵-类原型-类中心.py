...
def expand_features_with_cov(original_features, cov_matrix, num_per_sample):
    ...
    # 从生成的样本中随机选择200个补全样本 
    selected_indices = np.random.choice(all_generated_samples.shape[0], 30, replace=False) 
    return all_generated_samples[selected_indices]

# 合并自己的补全样本和其他客户端生成的样本，并确保原始样本保留
def combine_samples(original_features, expanded_features, other_generated_samples, target_size=120): 
    num_original = original_features.shape[0]  # 原始样本数量
    num_needed = target_size - num_original  # 需要从生成样本中选取的数量
    
    # 从合并的生成样本中选择需要的样本
    combined_generated_samples = np.vstack((expanded_features, other_generated_samples))
...
def process_clients(client_id, class_idx, original_features, cov_matrices, prototype_features, num_per_sample):
    ...
    # 2. 从其他客户端的类原型生成600个样本 
    additional_samples = []
    for prototype in prototype_features:  # 遍历其他客户端的类原型
        new_samples = generate_new_samples(prototype, cov_matrices[class_idx], 30) 
        additional_samples.append(new_samples)
    
    if len(additional_samples) > 0:
        additional_samples = np.vstack(additional_samples)  # 合并600个样本 
    else:
        print(f"没有找到其他客户端的原型进行生成，跳过类 {class_idx} 的其他样本生成。")
        additional_samples = np.empty((0, original_features.shape[1]))  # 空数组占位

    # 3. 合并补全样本和其他客户端生成的样本，确保原始样本保留
    final_samples = combine_samples(original_features, expanded_features, additional_samples, target_size=120) 
...
def main():
    ...
    datasets_config = {
        'Art': {'client_ids': [0], 'num_generated_per_sample': 30}, 
        'Clipart': {'client_ids': [1], 'num_generated_per_sample': 30}, 
        'Product': {'client_ids': [2], 'num_generated_per_sample': 30}, 
        'Real_World': {'client_ids': [3], 'num_generated_per_sample': 30}
    }
        ...
        for client_id in config['client_ids']:
            for class_idx in range(65):  # 修改为65个类别
                ...
                for other_dataset in other_datasets:
                    other_client_id = datasets_config[other_dataset]['client_ids'][0]  # 直接从配置中获取
