...
# 联邦训练和评估函数
def federated_train_and_evaluate(all_client_features, test_sets, communication_rounds, local_epochs, batch_size=16, learning_rate=0.001):
    ...
    report_path = './results/FedAvg_original_report.txt'
...
def plot_accuracies(accuracies, communication_rounds):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('./results/FedAvg_original_test_accuracy_plot.png')
    plt.show()

# 主函数
def main():
    # 客户端数据加载路径
    base_dir = './clip_office_home_train_features'  # 修改路径
