...
动机：我们使用常用的跨域数据集Digits、PACS、office_caltech_10开展工作，实验过程中，通过划分客户端成组，分配跨域数据集实现数据异质，对比2024cvpr&2023cvpr，按照3214的客户端数量划分组，每个客户端抽取该域数据集0.2的数据量，引入clip作为backbond（对比resnet10、resnet50）fedavg聚合效果98%，采取更极端划分，只分配0.01数据量，发现结果还是很高，修改划分客户端的数量为1111，数据量0.01，在各个数据域只划分一个客户端的高度域泛化情况下，训练效果仍然很高。
